...
export interface CreateIndicacaoRequest {
  // dados do consumidor a ser criado no banco
  id: string;
  username: string;
  nickname: string | null;
  full_name?: string | null;
  // dados do consumidor que indicou
  referral_id: string;
...
export class CreateIndicacaoUseCase {
  ...
  async execute(request: CreateIndicacaoRequest): Promise<CreateIndicacaoResponse> {
    ...
    const referrer = await this.cashbackConsumerRepository.findById(request.referral_id);
    if (!referrer) {
    ...
    request.referralCodeUsed = referrer.referral_code;
    // 4. Determinar o código de indicação do próprio novo consumidor

    const referral_code = randomUUID();
